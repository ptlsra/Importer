		...
		$.each(response.records, function(i, item) {
			...
			if(eventName=="ValidateAgreement"){
				 document.getElementById('importerUploadTime').innerHTML='  - '+convdataTime;
				  ...
				  document.getElementById('txId3').innerHTML='{<a href="#" onclick=check("ValidateAgreement")>'+item.transactionHash+'</a>}';
				  $("#txId3").fadeIn(2000);
			}
			if(eventName=="LOC Draft validated"){
				 document.getElementById('exporterValidateTime').innerHTML='  - '+convdataTime;
				  $("#exporterValidateTime").fadeIn(2000);
				  document.getElementById('txId4').innerHTML='{<a href="#" onclick=check("LOC_Draft_validated")>'+item.transactionHash+'</a>}';
				  $("#txId4").fadeIn(2000);
			}
			if(eventName=="LOC Document uploaded"){
				 document.getElementById('uploadLocTime').innerHTML='  - '+convdataTime;
				  $("#uploadLocTime").fadeIn(2000);
				  document.getElementById('txId5').innerHTML='{<a href="#" onclick=check("LOC_Document_uploaded")>'+item.transactionHash+'</a>}';
				  $("#txId5").fadeIn(2000);
			}
			if(eventName=="LOC Document validated"){
				 document.getElementById('finalApproval').innerHTML='  - '+convdataTime;
				  $("#finalApproval").fadeIn(2000);
				  document.getElementById('txId6').innerHTML='{<a href="#" onclick=check("LOC_Document_validated")>'+item.transactionHash+'</a>}';
				  $("#txId6").fadeIn(2000);
			}
  ...
  $.get("/getSalesAgreementDetails?agreementId="+agreementId, function(responseDataValue){
	 ...
	 //   $("#frame").attr("src", "http://"+ipfsIpAdd+":"+ipfsPort+"/ipfs/"+responseDataValue.agreementDocument);
	     ...
	     $('#btnShow2').click(function(){
	            ...
	            $.get("/ipfs?fileHash=" + responseDataValue.locDocument, function (response) {
